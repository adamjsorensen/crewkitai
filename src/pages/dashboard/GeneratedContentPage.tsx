...
const GeneratedContentPage = () => {
  ...
  const { data: generation, isLoading, error } = useQuery({
    ...
    onSuccess: (data) => {
      form.setValue('title', data.custom_prompt?.base_prompt?.title || `Generated Content ${new Date().toLocaleDateString()}`);
    },
          ...
          <div>
            ...
            {isSaveDialogOpen && (
              <Card className="mt-4">
                ...
                <CardContent>
                  <Form {...form}>
                    <form onSubmit={form.handleSubmit(handleSaveContent)} className="space-y-4">
                      <FormField
