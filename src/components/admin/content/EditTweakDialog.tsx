...
type EditTweakDialogProps = {
  tweak: ParameterTweak;
  parameters: PromptParameter[];
  open: boolean;
  onOpenChange: (open: boolean) => void;
};

const EditTweakDialog = ({ tweak, parameters, open, onOpenChange }: EditTweakDialogProps) => {
  const { updateParameterTweak } = useCrewkitPromptParameters();
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      name: tweak.name,
      parameterId: tweak.parameter_id || "",
      subPrompt: tweak.sub_prompt,
      order: tweak.order,
      active: tweak.active,
    },
  });
  ...
  const handleSubmit = async (values: FormValues) => {
    try {
      await updateParameterTweak.mutateAsync({
        id: tweak.id,
        name: values.name,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>Edit Parameter Tweak</DialogTitle>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
            ...
            <div className="flex justify-end space-x-2 pt-4">
              ...
              <Button type="submit">Update Tweak</Button>
            </div>
          </form>
        </Form>
      </DialogContent>
...
export default EditTweakDialog;
