...
>(({ className, ...props }, ref) => {
  ...
  React.useEffect(() => {
    console.log("[SheetOverlay] Rendered");
  }, []);
...
>(({ side = "right", className, children, ...props }, ref) => {
  // Log content rendering and props
  React.useEffect(() => {
    console.log("[SheetContent] Rendered with side:", side);
  }, [side]);
  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        ...
        onEscapeKeyDown={(e) => {
          console.log("[SheetContent] Escape key pressed");
      ...
      >
        ...
        <SheetPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-secondary"
          onClick={() => {
            console.log("[SheetContent] Close button clicked");
          }}
        >
          <X className="h-4 w-4" />
